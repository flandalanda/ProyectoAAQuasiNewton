...
while(iter < maxIter)
    ...
    phiPrimeA = dot(gnew, dk);
    
    % We check if any conditions have been violated
    if phiA > phi0 + c1*a1*phiPrime0 || (phiA >= phi0 && iter >1)
        [alpha, gnew] = zoom( a0, alpha, f, xk, fk, gTd);
        break
    elseif abs(phiPrime1) <= -c2*phiPrime0
        break
    elseif phiPrime1 >= 0
        [alpha, gnew] = zoom(alpha, a0, f, xk, dk, fk, phiPrime0);
        break;
    else
